...
export default function Navbar() {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/40 shadow-[0px_4px_4px_0px_#A4A4A440]">
            <div className="max-w-7xl mx-auto px-6 md:px-22.5 h-21.5 flex items-center justify-between">
                ...
                <div className="hidden lg:block">
                    {user ? (
                        ...
                        <Link 
                            href="/login"
                            className="flex items-center justify-center gap-2 w-30 h-9 rounded-[18px] border-[0.47px] border-brand-pink bg-brand-pink text-white text-sm font-bold hover:bg-brand-pink-hover transition-colors shadow-lg shadow-brand-pink/20"
                        >
            ...
            {/* Mobile Menu Overlay */}
            {isMobileMenuOpen && (
                <div className="lg:hidden absolute top-21.5 left-0 right-0 bg-background border-b border-border p-6 flex flex-col gap-6 shadow-xl animate-in slide-in-from-top-5">
